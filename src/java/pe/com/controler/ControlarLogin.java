 ...
 * @author User
 */
public class ControlarLogin extends HttpServlet {
private UsuarioDao udao = new UsuarioDaoImpl(); 
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!udao.login(username, clave).isEmpty()){ 
            ...
            sesion.setAttribute("nombre", lista.get(0).getNombre());
            sesion.setAttribute("rol", lista.get(0).getRol());
            
            String rol = lista.get(0).getRol();
            
            if ("Practicante".equals(rol)) { 
                response.sendRedirect("practicante.jsp");            
            } else {            
                response.sendRedirect("cordinador.jsp"); 
            }
